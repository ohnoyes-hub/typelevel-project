...
case class PostJobPage(
    ...
    override protected def renderFormContent(): List[Html[App.Msg]] = 
        ...
        renderToggle("Remote", "remote", false, _ => ToggleRemote),
        ...
        button(`class` := "form-submit-btn", `type` := "button", onClick(AttemptPostJob))(s"Post Job - €${Constants.jobAdvertPrice}"),
    )

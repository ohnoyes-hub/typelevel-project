...
final case class JobListPage(
    ...
    override def update(msg: App.Msg): (Page, Cmd[IO, App.Msg]) = msg match {
        case AddJobs(list, clm) => 
            (setSuccessStatus("Jobs loaded").copy(jobs = this.jobs ++ list, canLoadMore = clm), Cmd.None)
        case SetErrorStatus(msg) =>
            (setErrorStatus(msg), Cmd.None)
        case LoadMoreJobs =>
        ...
        div(`class` := "load-more-action")(
            s match {
                case Page.Status(_, Page.StatusKind.LOADING) => 
                    div("Loading...")
                case Page.Status(e, Page.StatusKind.ERROR) => 
                    div(e)
                case _ => 
                    if (canLoadMore) 
                        button(`type` := "button", onClick(LoadMoreJobs))("Load More")
                    else 
                        div("No more jobs to load")
            }
        )
    }
...
object JobListPage {
    ...
    // state
    case class SetErrorStatus(msg: String) extends Msg
        ...
        def getJobs(limit: Int = Constants.defaultPageSize, offset: Int = 0) = new Endpoint[Msg] {
            ...
            override val onResponse: Response => Msg = response => response.status match {
                case Status(s, _) if s >= 200 && s < 300 => 
                    ...
                    parsed match {
                        case Left(parsingError) => SetErrorStatus(parsingError.toString)
